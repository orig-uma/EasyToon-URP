...
- **顔 SDF に光の仰角フェード `Face SDF Elevation Fade Min / Max`**（T-440・試作）。顔 SDF は水平面内のスイープなので、頭が俯く・仰ぐ・トップライトが差す、のどれも「正面光」と読んでいた。俯いたときに顔だけ SDF で明るいまま残り、首（N·L）と HQ の落ち影（顎→首・前髪→額・鼻→上唇）だけが暗くなるので、落ち影が硬く浮いて見えていた。頭ローカルで見た光の仰角が Min（既定 0.35）を超えると法線の陰影へ戻り始め、Max（既定 0.8）で完全に戻る。通常経路は首と同じ伝達関数なので継ぎ目が合う。Min を 1.5 にすると従来どおり（仰角を無視）。
