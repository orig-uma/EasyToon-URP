...
- **顔 SDF の縦スイープ `Face SDF Vertical (BA)` / `Face Shadow Offset V`**（T-441・試作。**Core 0.3.4 相当（Unreleased）の `pack16Vertical` が要る**）。横スイープは水平面内なので光の仰角を知らず、頭が俯く・トップライトが差すと鼻下・唇・顎裏が「正面光」と読まれて明るいまま残り、首（N·L）との段差や落ち影の浮きになっていた。Baking タブの `Vertical Sweep (BA)` を ON にして焼くと、上光スイープ（正面 → 真上 → 背面）が B×256+A に 16bit で入り、シェーダーは頭ローカルの仰角だけを閾値にして読み、横と `min` で合成する（旧 4ch の加重平均は各軸の近くで線が痩せたので採らない）。下光は仰角 0 として読む（縦の寄与なし）。同じ 1 フェッチから取れるので追加のテクスチャ読みは無い。既定 0（従来の 1ch と同じ）。焼いたら Baking タブが 1 にする。
