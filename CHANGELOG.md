...
- **顔 SDF の縦スイープ `Face SDF Vertical (BA)` / `Face Shadow Offset V`**（T-441・試作。**Core 0.3.6 相当（Unreleased。0.3.5 の main を基点）の `pack16Vertical` が要る**）。横スイープは水平面内なので光の仰角を知らず、頭が俯く・トップライトが差すと鼻下・唇・顎裏が「正面光」と読まれて明るいまま残り、首（N·L）との段差や落ち影の浮きになっていた。Baking タブの `Vertical Sweep (BA)` を ON にして焼くと、上光スイープ（正面 → 真上 → 背面）が B×256+A に 16bit で入り、シェーダーは頭ローカルの仰角だけを閾値にして読み、横と `min` で合成する（旧 4ch の加重平均は各軸の近くで線が痩せたので採らない）。下光は仰角 0 として読む（縦の寄与なし）。同じ 1 フェッチから取れるので追加のテクスチャ読みは無い。既定 0（従来の 1ch と同じ）。焼いたら Baking タブが 1 にする。
- **顔 SDF の極の対処 `Face SDF Pole Fade`**（T-442・既定 0.9）。横スイープはライトを水平面に潰して方位角を取るので、真上・真下を通る瞬間（Directional の X が 90° / 270° 付近）に前後が入れ替わり、顔全体の影が一斉に反転していた。|光.y| がこの値を超えたら方位角の寄与を正面光（全面照射）へ滑らかに寄せる。上からの影は縦スイープが担当し、下からの光は焼いていないので照らされたまま（顎裏・鼻下は下光で照らされるので物理とも合う）。1 で無効（従来どおり）。
- **デバッグ表示に `FaceSDF H` / `FaceSDF V`**（T-441）。顔 SDF の横（R×256+G）と縦（B×256+A）をデコード後の値で表示する。縦を焼いていない 1ch テクスチャは BA = (0, 255) なので V がほぼ黒（0.004）になる ── その状態で `Face SDF Vertical` を上げると仰角 7° あたりから顔全体が影に落ちるので、V の表示で「焼けているか」を先に確かめる。
