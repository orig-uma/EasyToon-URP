...
- **顔 SDF の縦スイープ `Face SDF Vertical (BA)` / `Face Shadow Offset V` / `Face SDF Axis Fade`**（T-441 / T-443・試作。**Core 0.3.6 相当（Unreleased。0.3.5 の main を基点）の `bakeVertical` が要る**）。横スイープは水平面内なので光の仰角を知らず、頭が俯く・トップライト・下からの光で鼻下・唇・顎裏が「正面光」と読まれて明るいまま残り、首（N·L）との段差や落ち影の浮きになっていた。Baking タブの `Vertical Sweep (BA)` を ON にして焼くと、上光（B）と下光（A）のスイープが角度線形 8bit で入り、シェーダーは頭フレームの fwd-up 面内の角度で読んで横と `min` で合成する（旧 4ch の加重平均は各軸の近くで線が痩せたので採らない）。**横と縦を対称に扱う**: 横の方位角は真上・真下で、縦の角度は真横の光で定まらないので、各軸の閾値を面内成分の長さ（`Axis Fade` 未満で）照射側へ寄せる。どの経路でも片方の線が動き続け、極でも真横でも飛ばない。縦を使うときは横も頭フレームで取る（世界 XZ は直立時だけ一致する近似だった）。同じ 1 フェッチから取れるので追加のテクスチャ読みは無い。既定 0（従来の 1ch と同じ）。焼いたら Baking タブが 1 にする。
