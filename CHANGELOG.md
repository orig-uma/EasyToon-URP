...
- **顔 SDF の縦スイープ `Face SDF Vertical (BA)` / `Face Shadow Offset V` / `Face SDF Axis Fade`**（T-441 / T-443・試作。**Core 0.3.6 の `bakeVertical` が要る**（インストーラの Core ピンを v0.3.6 に））。横スイープは水平面内なので光の仰角を知らず、頭が俯く・トップライト・下からの光で鼻下・唇・顎裏が「正面光」と読まれて明るいまま残り、首（N·L）との段差や落ち影の浮きになっていた。Baking タブの `Vertical Sweep (BA)` を ON にして焼くと、上光（B）と下光（A）のスイープが角度線形 8bit で入り、シェーダーは頭フレームの fwd-up 面内の角度で読んで横と `min` で合成する（旧 4ch の加重平均は各軸の近くで線が痩せたので採らない）。**横と縦を対称に扱う**: 横の方位角は真上・真下で、縦の角度は真横の光で定まらないので、各軸の閾値を面内成分の長さ（`Axis Fade` 未満で）照射側へ寄せる。どの経路でも片方の線が動き続け、極でも真横でも飛ばない。縦を使うときは横も頭フレームで取る（世界 XZ は直立時だけ一致する近似だった）。同じ 1 フェッチから取れるので追加のテクスチャ読みは無い。既定 0（従来の 1ch と同じ）。焼いたら Baking タブが 1 にする。
- （没）`Face SDF Pole Fade`（T-442）は極の瞬間を連続にしただけで、「後ろ = 全面影」の切り替えがフェード窓に圧縮されて見えた。上の対称な重み付けで置き換えた。
- **デバッグ表示に `FaceSDF H` / `FaceSDF V Up` / `FaceSDF V Down`**（T-441）。顔 SDF の横（R×256+G デコード後）と縦の上・下（B / A）を表示する。縦を焼いていない 1ch テクスチャは B = 0 なので V Up が真っ黒になる ── その状態で `Face SDF Vertical` を上げると顔全体が影に落ちるので、先に「焼けているか」を確かめる。
- （没）光の仰角で法線の陰影へ戻す `Face SDF Elevation Fade` と、影を網点にする `Face Shadow Tone` は試作の上で撤去した（T-440）。前者は鼻・唇の凹凸を法線で出してしまい、後者は俯いたときに見えていた影がシャドウマップ由来ではなかったため効かなかった。仰角は縦スイープ（上）が担当する。

### Fixed

- **顎裏の SDF フェード（`Face SDF Blend Normal Min / Max`）が頭の回転に追従していなかった**（T-440）。判定がオブジェクト空間の法線 Y だったため、スキンメッシュでは頭が俯くと顎裏の法線が前を向いて「SDF 100%」側へ振れ、俯いたときほど顎裏〜首の段差が悪化していた。頭ボーンの up 軸（forward × right）との内積で判定するようにした。Binder が無いときはオブジェクトの Y に落ちるので従来と同じ。計算はライトに依存しないのでフラグメントで 1 回に畳んだ（Forward+ で灯数ぶん減る）。

### Changed

- **Face SDF ベイクの既定を変えた**: `Vertical Sweep (BA)` を ON（縦スイープが無いと `Face SDF Vertical` を上げたとき顔全体が影に落ちる。焼き直しで B / A も入るようにする）、`Proxy Shape` を卵型（Taper 0.3）から**楕円体**へ戻した（利用者判断。縦スイープを入れた後の見た目で判断）。どちらも Baking タブの初期値で、既に焼いたテクスチャは変わらない。
