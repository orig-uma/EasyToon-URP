...
- **顔の影トーン `Face Shadow Tone`（Off / Dots / Grain）**（T-440・試作）。顔に落ちるシャドウマップの影（俯いたときの鼻下・唇・前髪）を、遮蔽量を密度にした画面固定の網点に置き換える。PCF の滑らかな暗がりのままだと、SDF で平らにした顔の上で「実物の影」として浮いていた。`Dots` は Bayer 4×4 の網点、`Grain` はブルーノイズの粒。`Scale (px)` でセルの大きさ（既定 3）、`Strength` で点の濃さ（既定 0.6）、`Threshold` で薄い半影の切り捨て（既定 0.2）。漫画のトーンのように画面に固定されるので、カメラが動くと影の方が下を流れる。TAA では時間方向に均されて滑らかな半調に戻る。既定 Off。
- **顔 SDF に光の仰角フェード `Face SDF Elevation Fade Min / Max`**（T-440・試作・既定 OFF）。顔 SDF は水平面内のスイープなので、頭が俯く・仰ぐ・トップライトが差す、のどれも「正面光」と読む。頭ローカルで見た光の仰角が Min を超えると法線の陰影へ戻り始め、Max で完全に戻る。通常経路は首と同じ伝達関数なので継ぎ目が合う。**法線に戻すと鼻や唇の凹凸が出る**ので、顔を平らに保ちたい用途では OFF（Min 1.0 以上）のまま Face Shadow Tone を使う。
