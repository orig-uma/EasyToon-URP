...
Shader "Origuma/EasyToon_URP/Idol"
{
    Properties
    {
        ...
        // 極（真上・真下）の対処（T-442）。横スイープは L を水平面に潰して方位角を取るので、
        // ライトが真上・真下を通る瞬間に「前」と「後ろ」が入れ替わり、顔全体の影が一斉に
        // 反転する（Directional の X が 90° / 270° 付近）。|L.y| がこの値を超えたら方位角の
        // 寄与を「正面光（全面照射）」へ寄せて連続にする。上は縦スイープが影を担当し、
        // 下は焼いていないので照らされたまま。1 で無効（従来どおり反転する）。
        _FaceSDFPoleFade ("  Face SDF Pole Fade", Range(0.5,1)) = 0.9
