...
| FR-76 | ディテール法線に、ディテール色とは独立したタイリングを持たせる | `_DetailNormalMap_ST`。既存材質は `_MaterialVersion` 0 → 1 で Detail Map の値を写す |
| FR-77 | （欠番: 織り目の細かさをメッシュの UV 密度から切り離す UV1。T-430 で実装後、利用者判断で撤去） | ― |
| FR-78 | ディテール法線を回転でき、陰影の向きが付いてくる | `_DetailNormalRotation`。UV を R、法線 xy を Rᵀ |
